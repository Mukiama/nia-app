...
export default function Favourites() {
  ...
  async function handleRemove(favouriteId) {
    ...
    try {
      ...
      await authFetch(`/favourites/${favouriteId}`, { method: "DELETE" });
      setFavourites((prev) => prev.filter((item) => item.id !== favouriteId));
    } catch (err) {
  ...
  return (
    <main className="favourites-page">
      ...
      <section className="favourites-section">
        {loading && <div className="status-message">Loading favourites...</div>}
              ...
              <div className="place-card" key={item.id}>
                <Link to={`/places/${item.placeId}`}>
                  ...
                  <div className="place-card-content">
                    ...
                    <h2>{item.name}</h2>
                    <p className="place-location">{item.county}, Kenya</p>
                  </div>
                </Link>
